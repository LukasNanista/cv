...
namespace Pong
{
	class GameManager
	{
		public const int GoalsToReach = 5;
		...
		private char _lastPixel = CharacterUtilities.EmptyPixel;
		...
		private void SetGameMode()
		{
			...
			switch (GameMode)
			{
				...
				case GameMode.PVP:
					...
					UIUtilities.PrintPlayerInstructions(_firstPlayer.PlayerDataRef.Name, _secondPlayer.PlayerDataRef.Name);
					_scoreBoard = new ScoreDisplayHandler();
		...
		private void OnGoalScored()
		{
			...
			_lastPixel = CharacterUtilities.EmptyPixel;
		...
		{
			if (_lastPixel == CharacterUtilities.PlayerIcon)
				_board.GameField[_ball.PointRef.Y, _ball.PointRef.X] = _lastPixel;

			else if (_lastPixel == CharacterUtilities.TopBottomBorderIcon)
				_board.GameField[_ball.PointRef.Y, _ball.PointRef.X] = _lastPixel;
		}
