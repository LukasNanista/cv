namespace FancyPong
{
	class HighscoreManager
	{
		...
		private readonly string _path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Highscore.txt");
		...
		private void HighscoreWriter()
		{
			...
			{
				highscoreFile.WriteLine($"-{_winner.Name}- has beaten -{_loser.Name}- || {_winner.Score} to {_loser.Score} || {winningTime}");
			}
		}
			...
			if (File.Exists(_path))
			{
				...
				switch (key.Key)
				{
					case ConsoleKey.Delete:
						...
						Console.WriteLine("Your high scores have been deleted.");
						Console.ForegroundColor = ConsoleColor.White;
						File.Delete(_path);
						break;
					default:
						Console.WriteLine("Your high scores have been saved.");
						break;
				}
			}
			else
		...
		{
			int topForSetCursor = 7;
			int leftForSetCursor = 20;
